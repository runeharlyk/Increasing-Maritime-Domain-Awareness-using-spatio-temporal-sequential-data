...
from train_gru_advanced import AdvancedGRUTrajectoryPredictor

DATA_DIR = Path("parquet")
MODEL_PATH = "best_model_advanced.pt"
...
def predict_trajectories(model, sequences, input_scaler, output_scaler):
    ...
    with torch.no_grad():
        predictions, attention_weights = model(sequences_tensor)
...
def plot_trajectories(full_trajectories, predictions, mmsi_list, output_hours, timestamps_list):
    ...
    plt.tight_layout()
    plt.savefig("trajectory_predictions.png", dpi=300, bbox_inches="tight")
    print("\nSaved matplotlib plot to trajectory_predictions.png")
...
def main():
    ...
    predictions, attention_weights = predict_trajectories(model, sequences, input_scaler, output_scaler)
    ...
    print(f"  Matplotlib plot: trajectory_predictions.png")
