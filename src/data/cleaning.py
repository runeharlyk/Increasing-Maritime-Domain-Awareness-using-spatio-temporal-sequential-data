...
def clean_dataframe(df):
    ...
    if config.SPEED_ANOMALY_ACTION != "keep":
        print(
            f"  Filtering by point-to-point speed (min {config.MIN_POINT_TO_POINT_SPEED_KMH} km/h, max {config.MAX_POINT_TO_POINT_SPEED_KMH} km/h, action: {config.SPEED_ANOMALY_ACTION})..."
        )
        before = df.height
        ...
        for mmsi in unique_mmsi:
            ...
            for segment in segments:
                ...
                if avg_speed < config.MIN_POINT_TO_POINT_SPEED_KMH:
                    continue
        ...
        df = pl.concat(segment_dfs, how="vertical").sort(["MMSI", "Timestamp"])
        print(f"  Removed {before - df.height} rows from speed violations (too slow/fast). Rows now: {df.height}")
